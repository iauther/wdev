...
Biquad.prototype = Object.create( Biquad.prototype );
Biquad.prototype.constructor = Biquad;

Biquad.prototype.coefficients = function() {
  var b = [this.b0, this.b1, this.b2];
  var a = [this.a0, this.a1, this.a2];
  return {b: b, a:a};
};

Biquad.prototype.setFilterType = function(type) {
  this.type = type;
  this.calcCoef();
};

Biquad.prototype.setSampleRate = function(rate) {
  this.Fs = rate;
  this.calcCoef();
};

Biquad.prototype.setQ = function(q) {
  this.parameterType = DSP.Q;
  this.Q = Math.max(Math.min(q, 115.0), 0.001);
  this.calcCoef();
};

Biquad.prototype.setBW = function(bw) {
  this.parameterType = DSP.BW;
  this.BW = bw;
  this.calcCoef();
};

Biquad.prototype.setS = function(s) {
  this.parameterType = DSP.S;
  this.S = Math.max(Math.min(s, 5.0), 0.0001);
  this.calcCoef();
};

Biquad.prototype.setF0 = function(freq) {
  this.f0 = freq;
  this.calcCoef();
};

Biquad.prototype.setDbGain = function(g) {
  this.dBgain = g;
  this.calcCoef();
};

Biquad.prototype.calcCoef = function() {
  var A;

  if (this.type === DSP.PEAKING_EQ || this.type === DSP.LOW_SHELF || this.type === DSP.HIGH_SHELF ) {
  ...
  switch (this.type) {
    ...
    case DSP.LOW_SHELF:   // H(s) = A * (s^2 + (sqrt(A)/Q)*s + A)/(A*s^2 + (sqrt(A)/Q)*s + 1)
      coeff = sinw0 * Math.sqrt(A)/this.Q;
    ...
    case 'hl':
        var po = xo_conv(this.itemData.method,this.itemData.pre_order,this.itemData.response,this.Fs,this.f0);
        var len = po.length/6;
        this.a0 = [];
        this.a1 = [];
        this.a2 = [];
...
function xo_conv(method,pre_order,response,fs,freq) {
  var freq_tmp;
  var omega, f32dat;
  var pole_ptr;
  var order_tmp;
  var i, j, num_cnt= 0,out_cnt = 0;
  var pow = Math.pow,p = [],k = [],a = [],b = [],po = [];
  var BUTTERWORTH_POLE = [
    [[1,1]],
    [[1,1.4142,1]],
    [[1,1,1],[1,1]],
    [[1,0.7654,1],[1,1.8478,1]],
    [[1,0.6180,1],[1,1.6180,1],[1,1]],
    [[1,0.5176,1],[1,1.4142,1],[1,1.9319,1]]
  ];

  var BESSEL_POLE = [
    [[1,1]],
    [[1,1.73205080756888,1]],
    [[1,1.49128077169615,1.06202206013291],[1,0.94160002653321]],
    [[1,1.31442234334377,1.12109453233968],[1,1.80951759357649,0.89198544025814]],
    [[1,1.70310723873768,0.92114231954147],[1,1.18115188922384,1.17180404531632],[1,0.92644207738776]],
    [[1,1.81878136609445,0.86147458212267],[1,1.59930837166566,0.95548387620852],[1,1.07710536333862,1.21488220055987]]
  ];
  ...
  freq_tmp = freq;
  if(order_tmp!=0) {
    if(method == DSP.BESSEL){   //bessel 频率矫正
      switch(response) {
        case DSP.LOWPASS:
          switch(order_tmp) {
            case 1:
    ...
    omega = Math.tan(Math.PI*freq_tmp/fs);

    switch(method) {
      case DSP.BUTTERWORTH:
        pole_ptr = BUTTERWORTH_POLE;
        break;

      case DSP.BESSEL:
        pole_ptr=BESSEL_POLE;
        break;

      case DSP.LINKWITZ:
        pole_ptr = LINKWITZ_POLE;
        if(order_tmp%2)pole_ptr = BUTTERWORTH_POLE;
        break;
    }

    for(j=order_tmp,num_cnt=0;j>0;num_cnt++) {
      if(j>=2)
      {   //2阶
        switch (response)//[4:3]
        {
          case DSP.LOWPASS:      //LP
            p[0] = pow(omega,2);
            f32dat = pole_ptr[order_tmp-1][num_cnt][0];
            k[0] = f32dat*pow(omega,2);
            p[1] = 0;
            f32dat = pole_ptr[order_tmp-1][num_cnt][1];
            k[1] = f32dat*pow(omega,1);
            p[2]=0;
            f32dat = pole_ptr[order_tmp-1][num_cnt][2];
            k[2] = f32dat*pow(omega,0);
            break;

          case DSP.HIGHPASS:       //HP
            p[0] = 0;
            f32dat = pole_ptr[order_tmp-1][num_cnt][2];
            k[0] = f32dat*pow(omega,2);
            p[1] = 0;
        ...
        po[out_cnt++] = a[0];  //a0
        po[out_cnt++] = b[0];  //b0
        po[out_cnt++] = b[1];//a[0];  //b1
        po[out_cnt++] = a[1];  //a1
        po[out_cnt++] = b[2];//a[0];  //b2
        po[out_cnt++] = a[2];  //a2
      }
      else if(j==1)
      {   //1阶
        switch (response)
        {
          case DSP.LOWPASS:      //LP
            p[0] = pow(omega,1);
            k[0] = pole_ptr[order_tmp-1][num_cnt][0]*pow(omega,1);
            p[1] = 0;
            k[1] = pole_ptr[order_tmp-1][num_cnt][1]*pow(omega,0);
            break;

          case DSP.HIGHPASS:       //HP
            p[0] = 0;
            k[0] = pole_ptr[order_tmp-1][num_cnt][1]*pow(omega,1);
            p[1] = 1;
            k[1] = 1;
            break;
        }
...
Biquad.prototype.processStereo = function(buffer) {
  //y[n] = (b0/a0)*x[n] + (b1/a0)*x[n-1] + (b2/a0)*x[n-2]
  //       - (a1/a0)*y[n-1] - (a2/a0)*y[n-2]
...
DSP.mag2db = function(buffer) {
  ...
  var log10 = Math.log10 ? Math.log10 : mathLog10;
  var max = Math.max;
  ...
  for (var i=0; i<buffer.length; i++) {
    //tmp = 20.0*log10(max(buffer[i], minMag));
    tmp = 20.0*log10(buffer[i]);
    //console.log('____ buffer[%d]: %f', i, buffer[i]);
    //console.log('____ minMag: %f', minMag);
    //console.log('____ result[%d]: %f', i, result[i]);
    result[i] = gain2y(tmp, 60, -30, 30);                               //need checking again
  }
...
//console.log('___2____ a: %f, %f, %f', a[0], a[1], a[2]);
//console.log('___2____ b: %f, %f, %f', b[0], b[1], b[2]);
  ...
  for (i=0; i<w.length; i++) {
    ...
    //console.log('____  numerator.real: %f, %f', numerator.real, numerator.imag);
    //console.log('____  denominator.real: %f, %f', denominator.real, denominator.imag);
 ...
 *  Freq Equalizer
 *
 *  guohui added 2016/6/29
 *  rates: sampleRate;
 *  oct: 1,3,6 etc.
 */
function FrequencyEq(type,rates,peak) {
  this.FS = rates;
  this.type = type;
  this.peak = peak;
  this.minFreq = 20.0;
...
FrequencyEq.prototype = Object.create( FrequencyEq.prototype );
FrequencyEq.prototype.constructor = FrequencyEq;

FrequencyEq.prototype.calcFilter = function(type) {
  if(!this.filter || this.type != type){
    this.filter = new Biquad(this.type, this.FS);
    this.filter.itemData = this.itemData;
    this.filter.setDbGain(this.peak.gain);
    this.filter.setQ(this.peak.Q);
    this.filter.setF0(this.peak.freq);
  }
};

FrequencyEq.prototype.setType = function(type,data) {
  this.type = type;
  this.itemData = data;
  this.calcFilter();
};

FrequencyEq.prototype.setFreq = function(freq) {
  this.filter.setF0(freq);
  this.calcFilter();
};

FrequencyEq.prototype.setQ = function(Q) {
  this.filter.setQ(Q);
  this.calcFilter();
};


FrequencyEq.prototype.setGain = function(gain) {
  this.filter.setDbGain(gain);
  this.calcFilter();
};


function x2freq(xy,xylen,vmin,vmax) {
  var f=0,step,log10 = Math.log10 ? Math.log10 : mathLog10;
  if (vmin==0) {
    vmin += 1;
    vmax += 1;
  }

  step = xylen/(log10(vmax)-log10(vmin));
  f = Math.pow(10, (xy+step*log10(vmin))/step);
...
gain2y = function(v,xylen,vmin,vmax) {
  return (v*xylen/(vmax-vmin));
};



FrequencyEq.prototype.calcFreqz = function(freqs) {
  var a, b,buf;
  freqs = Math.floor(freqs);
  if (!this.buffer || this.buffer.length!=freqs) {

    var freq, freq_len = freqs;
    this.buffer = new Float64Array(freq_len);
    for (var i=0; i<freq_len; i++) {
      //this.buffer[i] = Math.PI/this.buffer.length*i;
      freq = x2freq(i, freq_len, 20, 20000);

      this.buffer[i] = DSP.TWO_PI * freq / this.filter.Fs;
    }

  }
  if(this.filter.a0 instanceof Array){
    var tmpArray = [];
    for(var i= 0,len=this.filter.a0.length;i<len;i++){
      a = [this.filter.a0[i], this.filter.a1[i], this.filter.a2[i]];
      b = [this.filter.b0[i], this.filter.b1[i], this.filter.b2[i]];

      buf = DSP.freqz(b, a, this.buffer);

      tmpArray = DSP.mag2db(buf);

      if(i == 0){
        this.freqzs = tmpArray;
      }else{
        for(var j= 1,olen=tmpArray.length;j<olen;j++){
          this.freqzs[j] += tmpArray[j];
        }
      }
    }
  }else{
    a = [this.filter.a0, this.filter.a1, this.filter.a2];
    b = [this.filter.b0, this.filter.b1, this.filter.b2];

    buf = DSP.freqz(b, a, this.buffer);

    this.freqzs = DSP.mag2db(buf);
  }

  /*测试freq对应的gain值*/
  //var point0 = Math.floor(CalcXPos6(freqs,20,20000,20));
  //var point1 = Math.floor(CalcXPos6(freqs,20,20000,100));
  //var point2 = Math.floor(CalcXPos6(freqs,20,20000,1000));
  //var point3 = Math.floor(CalcXPos6(freqs,20,20000,10000));
  //var point4 = Math.floor(CalcXPos6(freqs,20,20000,20000))-1;
  //var f0 = 50;
  //var g = 10;
  //var q = .7;
  //if(this.filter.f0 == f0 && this.filter.dBgain == g && this.filter.Q == q){
  //  console.log(this.freqzs[point0],this.freqzs[point1],this.freqzs[point2],this.freqzs[point3],this.freqzs[point4]);
  //}
};

FrequencyEq.prototype.process = function(buffer) {
  return this.filter.process(buffer);
};

FrequencyEq.prototype.processStereo = function(buffer) {
  return this.filter.processStereo(buffer);
};
...
function CalcXPos6( width, sFreq, eFreq, freq) //根据输入的参数计算出在X坐标的位置
{
  var step = 0,fLog = 0;
  var log10 = Math.log10 ? Math.log10 : mathLog10;
  ...
  if (sFreq == 0)
  {
    ...
    eFreq = eFreq + 1;
    freq = freq + 1;
    step = (width) / (log10(eFreq) - log10(sFreq));
    fLog = (log10(freq) - log10(sFreq));
  }
  else
  {
    step = (width) / (log10(eFreq) - log10(sFreq));
...
function CalcFreq6(width, x, sFreq, eFreq)//根据输入的坐标值计算对数关系的频率值
{
  var freq = 0,step,log10 = Math.log10 ? Math.log10 : mathLog10;
  if (sFreq == 0)
  {
    sFreq = sFreq + 1;
    eFreq = eFreq + 1;
    step = (width) / (log10(eFreq) - log10(sFreq));
    freq = Math.pow(10, (x + step * log10(sFreq)) / step);
    freq = freq - 1;
  }
  else
  {
...
function CalcQPos(q) {
  ...
  if (q >= 0 && q <= 26 / all) {
    result = q * all * .1 + .1;
  } else if (q > 26 / all && q <= 36 / all) {
    result = ((q - 26 / all) / (10 / all)) * 2 + 3;
  } else if (q > 36 / all && q <= 46 / all) {
    result = ((q - 36 / all) / (10 / all)) * 5 + 5;
  } else if (q > 46 / all && q <= 66 / all) {
    result = ((q - 46 / all) / (20 / all)) * 20 + 10;
  } else if (q > 66 / all && q <= 1) {
    result = ((q - 66 / all) / (9.8 / all)) * 98 + 30;
  }
  result = result * 10;
  return result;
}
...
function CalcQ(q){
  ...
  if(q >= .1 && q <= 3){
    ...
    result = ((q-3)/2)*10/all+26/all;
  }else if(q >5 && q <= 10){
    result = ((q-5)/5)*10/all+36/all;
  }else if(q >10 && q <= 30){
    result = ((q-10)/20)*20/all+46/all;
  }else if(q >30 && q <= 128){
    result = ((q-30)/98)*9.8/all+66/all;
  }
  return result;
}

function cpsRatioVal2y(val,per){
  return (1-per/val);
}

function cpsRatioY2val(y,per){
  var val;
  if(y == 1){
      val = per*25;
  }else {
  ...
  if(val > 25*per)val = 25*per;
  return val;
}

function mathLog10(val){
